...
use rustc_hir::{
  def::{DefKind, NonMacroAttrKind, Res},
...
impl Slice__PathSegmentDef {
  ...
  {
    #[derive(Serialize)]
    struct Wrapper<'a>(#[serde(with = "PathSegmentDef")] &'a PathSegment<'a>);
...
impl Option__GenericArgsDef {
  fn serialize<'hir, S>(
    value: &Option<&'hir GenericArgs<'hir>>,
    s: S,
  ) -> Result<S::Ok, S::Error>
  where
    S: Serializer,
...
impl Slice__GenericArgDef {
  ...
  {
    #[derive(Serialize)]
    struct Wrapper<'a>(#[serde(with = "GenericArgDef")] &'a GenericArg<'a>);
    serialize_custom_seq! { Wrapper, s, value}
  }
}
...
impl Slice__TyDef {
  ...
  {
    #[derive(Serialize)]
    struct Wrapper<'a>(#[serde(with = "TyDef")] &'a Ty<'a>);
    serialize_custom_seq! { Wrapper, s, value}
  }
}
...
pub enum TyKindDef<'hir> {
  InferDelegation(#[serde(skip)] DefId, #[serde(skip)] InferDelegationKind),

  Slice(#[serde(with = "TyDef")] &'hir Ty<'hir>),

  Array(
    #[serde(with = "TyDef")] &'hir Ty<'hir>,
    #[serde(skip)] ArrayLen,
  ),

  Ptr(#[serde(with = "MutTyDef")] MutTy<'hir>),

  Ref(
    #[serde(with = "LifetimeDef")] &'hir Lifetime,
    #[serde(with = "MutTyDef")] MutTy<'hir>,
  ),

  BareFn(#[serde(skip)] &'hir BareFnTy<'hir>),

  Never,

  Tup(#[serde(with = "Slice__TyDef")] &'hir [Ty<'hir>]),

  Path(#[serde(with = "QPathDef")] QPath<'hir>),

  OpaqueDef(
    #[serde(skip)] ItemId,
    #[serde(skip)] &'hir [GenericArg<'hir>],
    #[serde(skip)] bool,
  ),

  TraitObject(
    #[serde(with = "Slice__PolyTraitRefDef")] &'hir [PolyTraitRef<'hir>],
    #[serde(with = "LifetimeDef")] &'hir Lifetime,
    #[serde(skip)] TraitObjectSyntax,
  ),

  // NOTE: after reading the documentation I'm still not sure what this is.
  Typeof(#[serde(skip)] AnonConst),
...
pub enum QPathDef<'hir> {
  Resolved(
    #[serde(with = "Option__TyDef")] Option<&'hir Ty<'hir>>,
    #[serde(with = "PathDef")] &'hir Path<'hir>,
  ),
  TypeRelative(
    #[serde(with = "TyDef")] &'hir Ty<'hir>,
    #[serde(with = "PathSegmentDef")] &'hir PathSegment<'hir>,
  ),
  LangItem(#[serde(skip)] LangItem, #[serde(skip)] Span),
}
...
impl Slice__TypeBindingDef {
  ...
  {
    #[derive(Serialize)]
    struct Wrapper<'a>(#[serde(with = "TypeBindingDef")] &'a TypeBinding<'a>);
    serialize_custom_seq! { Wrapper, s, value}
  }
}
...
pub struct Option__ImplDef;
impl Option__ImplDef {
  pub fn serialize<'hir, S>(
    value: &Option<&'hir Impl<'hir>>,
    s: S,
  ) -> Result<S::Ok, S::Error>
  where
    S: Serializer,
  {
    match value {
      None => NOOP.serialize(s),
      Some(ga) => ImplDef::serialize(ga, s),
    }
  }
}
...
impl Slice__GenericParamDef {
  ...
  {
    #[derive(Serialize)]
    struct Wrapper<'a>(#[serde(with = "GenericParamDef")] &'a GenericParam<'a>);
    serialize_custom_seq! { Wrapper, s, value}
  }
}
...
impl Slice__WherePredicateDef {
    ...
    S: Serializer,
  {
    #[derive(Serialize)]
    struct Wrapper<'a>(
      #[serde(with = "WherePredicateDef")] &'a WherePredicate<'a>,
    );
    serialize_custom_seq! { Wrapper, s, value}
  }
}
...
pub enum WherePredicateDef<'hir> {
  BoundPredicate(
    #[serde(with = "WhereBoundPredicateDef")] WhereBoundPredicate<'hir>,
  ),
  RegionPredicate(
    #[serde(with = "WhereRegionPredicateDef")] WhereRegionPredicate<'hir>,
  ),
  EqPredicate(#[serde(with = "WhereEqPredicateDef")] WhereEqPredicate<'hir>),
}
...
pub enum GenericBoundDef<'hir> {
  Trait(
    #[serde(with = "PolyTraitRefDef")] PolyTraitRef<'hir>,
    #[serde(with = "TraitBoundModifierDef")] TraitBoundModifier,
  ),
  Outlives(#[serde(with = "LifetimeDef")] &'hir Lifetime),
}
...
impl Slice__GenericBoundDef {
  ...
  {
    #[derive(Serialize)]
    struct Wrapper<'a>(#[serde(with = "GenericBoundDef")] &'a GenericBound<'a>);
    serialize_custom_seq! { Wrapper, s, value}
  }
}
...
impl Slice__PolyTraitRefDef {
  ...
  {
    #[derive(Serialize)]
    struct Wrapper<'a>(#[serde(with = "PolyTraitRefDef")] &'a PolyTraitRef<'a>);
    serialize_custom_seq! { Wrapper, s, value}
  }
}
...
pub enum ResDef<Id = HirId> {
  Def(#[serde(skip)] DefKind, #[serde(skip)] DefId),
  PrimTy(#[serde(skip)] PrimTy),
  SelfTyParam {
    #[serde(skip)]
    trait_: DefId,
  },
  ...
  SelfCtor(#[serde(skip)] DefId),
  Local(#[serde(skip)] Id),
  ToolMod,
  NonMacroAttr(#[serde(skip)] NonMacroAttrKind),
  Err,
}
