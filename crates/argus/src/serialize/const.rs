...
pub enum ConstKind__TyCtxt<'tcx> {
  Param(
    #[serde(with = "ParamConstDef")] <TyCtxt<'tcx> as Interner>::ParamConst,
  ),
  Infer(#[serde(with = "InferConstDef")] InferConst),
  Bound(
    #[serde(skip)] DebruijnIndex,
    #[serde(with = "BoundConstDef")] <TyCtxt<'tcx> as Interner>::BoundConst,
  ),
  Placeholder(
    #[serde(with = "PlaceholderConstDef")]
    <TyCtxt<'tcx> as Interner>::PlaceholderConst,
  ),
  Unevaluated(
    #[serde(with = "AliasConstDef")] <TyCtxt<'tcx> as Interner>::AliasConst,
  ),
  Value(
    #[serde(with = "ValueConstDef")] <TyCtxt<'tcx> as Interner>::ValueConst,
  ),
...
impl<'tcx> From<&ir::ConstKind<TyCtxt<'tcx>>> for ConstKind__TyCtxt<'tcx> {
  fn from(value: &ir::ConstKind<TyCtxt<'tcx>>) -> Self {
    match value {
      ...
      ConstKind::Bound(v1, v2) => {
        ConstKind__TyCtxt::Bound(v1.clone(), v2.clone())
      }
