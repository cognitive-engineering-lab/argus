...
pub fn try_print_trimmed_def_path(&mut self, def_id: DefId) -> bool {
  ...
  pub fn path_crate(&mut self, cnum: CrateNum) {
    ...
    if cnum == LOCAL_CRATE {
        ...
        self.segments.push(PathSegment::unambiguous_name(self.tcx().crate_name(cnum)));
        self.empty_path = false;
    }
}
...
fn generic_delimiters(
    ...
    self.segments.push(PathSegment::GenericDelimiters { inner: after, })
    // CHANGE: write!(self, ">")?;
}
        ...
        // T: Print<'tcx, Self>,
    {
        ...
        self.segments.push(PathSegment::CommaSeparated {
            entries: elems.map(|e| {
                serde_json::to_value(e).expect("failed to serialize comma separated value")
            }).collect::<Vec<_>>(),
            kind,
        });
    }
