...
impl<'a, 'tcx: 'a> FnCtxtSimulator<'a, 'tcx> {
  pub fn new(
...
impl<'a, 'tcx: 'a> FnCtxtExt<'tcx> for FnCtxtSimulator<'a, 'tcx> {
  ...
  fn adjust_fulfillment_error_for_expr_obligation(
    ...
    let (expr, qpath) = match self.tcx.hir_node(hir_id) {
      hir::Node::Expr(expr) => {
        ...
        let qpath = if let hir::ExprKind::Path(qpath) = expr.kind {
          Some(qpath)
        } else {
          None
        };
  ...
  fn point_at_generic_if_possible(
    ...
    let Some((index, _)) = own_args
      .iter()
      .enumerate()
      .find(|(_, arg)| **arg == param_to_point_at)
    else {
      return false;
    };
  ...
  fn find_ambiguous_parameter_in<T: TypeVisitable<TyCtxt<'tcx>>>(
    ...
    struct FindAmbiguousParameter<'a, 'tcx>(
      &'a FnCtxtSimulator<'a, 'tcx>,
      DefId,
    );
    ...
    t.visit_with(&mut FindAmbiguousParameter(self, item_def_id))
      .break_value()
  }
  ...
  fn point_at_field_if_possible(
    ...
    if let [field] = fields_referencing_param.as_slice() {
      for expr_field in expr_fields {
        ...
        if self.tcx.adjust_ident(expr_field.ident, variant_def_id)
  ...
  fn blame_specific_expr_if_possible(
    ...
    let expr = match self
      .blame_specific_expr_if_possible_for_obligation_cause_code(
  ...
  fn blame_specific_expr_if_possible_for_derived_predicate_obligation(
    ...
    // that struct type.
    let impl_trait_self_ref =
      if self.tcx.is_trait_alias(obligation.impl_or_alias_def_id) {
        ty::TraitRef::new(
          self.tcx,
          obligation.impl_or_alias_def_id,
          ty::GenericArgs::identity_for_item(
            self.tcx,
            obligation.impl_or_alias_def_id,
          ),
        )
    ...
    match impl_predicates.predicates[impl_predicate_index]
      .0
      .kind()
      .skip_binder()
    {
  ...
  fn blame_specific_part_of_expr_corresponding_to_generic_param(
      ...
      (&expr.kind, in_ty.kind())
    {
      ...
      let Some((drill_expr, drill_ty)) = is_iterator_singleton(
        expr_elements.iter().zip(in_ty_elements.iter()).filter(
          |(_expr_elem, in_ty_elem)| {
    ...
    ) = (&expr.kind, in_ty.kind())
    {
      ...
      let Some((drill_generic_index, generic_argument_type)) =
        is_iterator_singleton(
          in_ty_adt_generic_args.iter().enumerate().filter(
            |(_index, in_ty_generic)| find_param_in_ty(*in_ty_generic, param),
          ),
        )
      else {
        return Err(expr);
      ...
      let expr = self
        .blame_specific_part_of_expr_corresponding_to_generic_param(
          param_to_point_at_in_struct,
          field_expr,
          field_type.into(),
        )?;
    ...
    ) = (&expr.kind, in_ty.kind())
    {
      ...
      let variant_def_id = match expr_struct_def_kind {
        ...
        DefKind::Ctor(hir::def::CtorOf::Variant, hir::def::CtorKind::Fn) => {
          ...
          // FIXME: Deal with type aliases?
          if in_ty_adt.did()
            == self.tcx.parent(self.tcx.parent(expr_ctor_def_id))
          {
